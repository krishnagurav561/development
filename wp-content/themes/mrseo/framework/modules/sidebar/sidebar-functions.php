...
if (!function_exists('mrseo_elated_sidebar_layout')) {
	/**
	...
	function mrseo_elated_sidebar_layout() {
		...
		if ( ! empty( $sidebar_layout ) && ! is_active_sidebar( mrseo_elated_get_sidebar() ) ) {
			$sidebar_layout = '';
		}
...
if(!function_exists('mrseo_elated_get_content_sidebar_class')) {
	/**
	 * Return classes for content holder when sidebar is active
	 *
	 * @return string
	 */
	function mrseo_elated_get_content_sidebar_class() {
		...
		return mrseo_elated_get_class_attribute($content_class);
	}
}

if(!function_exists('mrseo_elated_get_sidebar_holder_class')) {
	/**
...
if (!function_exists('mrseo_elated_get_sidebar')) {
	/**
	 * Return Sidebar name
	 *
	 * @return string
	 */
	function mrseo_elated_get_sidebar() {
		$sidebar_name = 'sidebar';
		$custom_sidebar_area = mrseo_elated_get_meta_field_intersect('custom_sidebar_area');
		$custom_archive_sidebar_area = mrseo_elated_options()->getOptionValue('archive_custom_sidebar_area');
		$custom_search_sidebar_area = mrseo_elated_options()->getOptionValue('search_custom_sidebar_area');
		$custom_single_sidebar_area = mrseo_elated_get_meta_field_intersect('blog_single_custom_sidebar_area');
		...
		if ((is_archive() || (is_home() && is_front_page())) && !mrseo_elated_is_woocommerce_page() && !empty($custom_archive_sidebar_area)) {
		...
		if (is_archive() && mrseo_elated_is_woocommerce_installed()) {
			if (is_product_category() || is_product_tag()) {
				$shop_id = get_option('woocommerce_shop_page_id');
				$sidebar_name = mrseo_elated_get_meta_field_intersect('custom_sidebar_area', $shop_id);
			}
		}
...
if (!function_exists('mrseo_elated_get_custom_sidebars')) {
	/**
	 ...
	 */
	function mrseo_elated_get_custom_sidebars() {
		$mrseo_custom_sidebars = get_option('eltdf_sidebars');
		$formatted_array = array();
		
		if (is_array($mrseo_custom_sidebars) && count($mrseo_custom_sidebars)) {
			foreach ($mrseo_custom_sidebars as $custom_sidebar) {
