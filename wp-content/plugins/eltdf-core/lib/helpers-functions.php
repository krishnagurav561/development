...
if ( class_exists( 'WP_Block_Type' ) && defined( 'ELATED_ROOT' ) ) {
	add_action( 'admin_head', 'mrseo_elated_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'mrseo_elated_meta_box_add' );
}

if ( ! function_exists( 'mrseo_elated_create_meta_box_handler' ) ) {
	function mrseo_elated_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'eltdf-meta-box-'.$key,
			$box->title,
