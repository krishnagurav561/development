...
class SocialShare implements Lib\ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
        ...
        $params['enable_social_share'] = (mrseo_elated_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

        //Is social share enabled for post type
        $post_type = get_post_type();
        $params['enabled'] = (mrseo_elated_options()->getOptionValue('enable_social_share_on_' . $post_type) == 'yes') ? true : false;
    ...
    private function getSocialNetworksParams($params) {
        ...
        foreach ($this->socialNetworks as $net) {
            ...
            $html = '';
            if (mrseo_elated_options()->getOptionValue('enable_' . $net . '_share') == 'yes') {
                ...
                $params['custom_icon'] = (mrseo_elated_options()->getOptionValue($net . '_icon')) ? mrseo_elated_options()->getOptionValue($net . '_icon') : '';
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (mrseo_elated_options()->getOptionValue('twitter_via') !== '') ? ' via ' . mrseo_elated_options()->getOptionValue('twitter_via') . ' ' : '';
                $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(mrseo_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . mrseo_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
