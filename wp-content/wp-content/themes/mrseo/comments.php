...
<?php if ( comments_open() || get_comments_number()) { ?>
	...
	$eltdf_args = array(
		...
		'comment_field' => '<textarea id="comment" placeholder="'.esc_attr__( 'Your comment','mrseo' ).'" name="comment" cols="45" rows="6" aria-required="true"></textarea>',
		'comment_notes_before' => '',
		'comment_notes_after' => '',
		'fields' => apply_filters( 'comment_form_default_fields', array(
			'author' => '<div class="eltdf-grid-row eltdf-grid-tiny-gutter"><div class="eltdf-grid-col-6"><input id="author" name="author" placeholder="'. esc_attr__( 'Your Name','mrseo' ) .'" type="text" value="' . esc_attr( $eltdf_commenter['comment_author'] ) . '"' . $eltdf_aria_req . ' /></div>',
			'email' => '<div class="eltdf-grid-col-6"><input id="email" name="email" placeholder="'. esc_attr__( 'Your Email','mrseo' ) .'" type="text" value="' . esc_attr(  $eltdf_commenter['comment_author_email'] ) . '"' . $eltdf_aria_req . ' /></div></div>',
			'cookies' => '<p class="comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes" ' . $eltdf_consent . ' />' .
				'<label for="wp-comment-cookies-consent">' . esc_html__( 'Save my name, email, and website in this browser for the next time I comment.', 'mrseo' ) . '</label></p>',
		) ) );
	 ?>
