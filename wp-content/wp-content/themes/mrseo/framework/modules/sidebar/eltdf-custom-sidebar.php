...
if (!class_exists('MrSeoElatedSidebar')) {

    class MrSeoElatedSidebar
    {
        ...
        function __construct() {
            ...
            $this->title = esc_html__('Custom Widget Area', 'mrseo');
        ...
        function load_assets() {
            ...
            wp_enqueue_script('eltdf-sidebar', ELATED_ROOT . '/framework/admin/assets/js/eltdf-sidebar.js');
            wp_enqueue_style('eltdf-sidebar', ELATED_ROOT . '/framework/admin/assets/css/eltdf-sidebar.css');
        }

        //widget form template
        function template_add_widget_field() {
            $nonce = wp_create_nonce('eltdf-delete-sidebar');
            $nonce = '<input type="hidden" name="eltdf-delete-sidebar" value="' . $nonce . '" />';

            echo "\n<script type='text/html' id='eltdf-add-widget'>";
            echo "\n  <form class='eltdf-add-widget' method='POST'>";
            echo "\n  <h3>" . esc_html($this->title) . "</h3>";
            echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'mrseo') . "' name='eltdf-add-widget' /></span>";
            echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'mrseo') . "' />";
            echo "\n    " . $nonce;
            echo "\n  </form>";
            echo "\n</script>\n";
        }
        ...
        function register_custom_sidebars() {
            ...
            $args = apply_filters('mrseo_elated_custom_widget_args', $args);
