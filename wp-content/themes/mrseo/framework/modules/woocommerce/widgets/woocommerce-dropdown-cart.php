					...
					<div class="eltdf-shopping-cart-dropdown">
							...
							<?php foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
								...
								<li>
									...
									<div class="eltdf-item-info-holder">
										...
										<?php echo apply_filters( 'mrseo_elated_woo_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s"><span class="icon-arrows-remove"></span></a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_html__('Remove this item', 'mrseo') ), $cart_item_key ); ?>
									</div>
								</li>
							<?php endforeach; ?>
						</ul>
						<div class="eltdf-cart-bottom">
							...
							<div class="eltdf-btn-holder clearfix">
								<a itemprop="url" href="<?php echo esc_url(wc_get_checkout_url()); ?>" class="eltdf-checkout" data-title="<?php esc_attr_e('CHECKOUT','mrseo'); ?>"><span><?php esc_html_e('CHECKOUT','mrseo'); ?></span></a>
								<a itemprop="url" href="<?php echo esc_url(wc_get_cart_url()); ?>" class="eltdf-view-cart" data-title="<?php esc_attr_e('VIEW CART','mrseo'); ?>"><span><?php esc_html_e('VIEW CART','mrseo'); ?></span></a>
							</div>
						</div>
					</div>
				<?php else : ?>
					...
					<?php foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
						...
						<li>
							...
							<div class="eltdf-item-info-holder">
								<h5 itemprop="name" class="eltdf-product-title"><a itemprop="url" href="<?php echo esc_url(get_permalink( $cart_item['product_id'] )); ?>"><?php echo apply_filters('mrseo_elated_woo_widget_cart_product_title', $_product->get_title(), $_product ); ?></a></h5>
								<?php echo apply_filters( 'mrseo_elated_woo_cart_item_price_html', wc_price( $product_price ), $cart_item, $cart_item_key ); ?>
								<?php echo apply_filters( 'mrseo_elated_woo_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s"><span class="fa fa-times"></span></a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_html__('Remove this item', 'mrseo') ), $cart_item_key ); ?>
							</div>
						</li>
					<?php endforeach; ?>
				</ul>
