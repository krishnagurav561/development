...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Elated Product Info', 'mrseo' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'mrseo' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'mrseo' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'mrseo' ),
							'value'       => array(
								esc_html__( 'Title', 'mrseo' )          => 'title',
								esc_html__( 'Featured Image', 'mrseo' ) => 'featured_image',
								esc_html__( 'Category', 'mrseo' )       => 'category',
								esc_html__( 'Excerpt', 'mrseo' )        => 'excerpt',
								esc_html__( 'Price', 'mrseo' )          => 'price',
								esc_html__( 'Rating', 'mrseo' )         => 'rating',
								esc_html__( 'Add To Cart', 'mrseo' )    => 'add_to_cart',
								esc_html__( 'Tag', 'mrseo' )            => 'tag',
								esc_html__( 'Author', 'mrseo' )         => 'author',
								esc_html__( 'Date', 'mrseo' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'mrseo' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'mrseo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'mrseo' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'mrseo' ),
							'value'       => array_flip( mrseo_elated_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'mrseo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'mrseo' ),
							'value'      => array(
								esc_html__( 'Default', 'mrseo' ) => '',
								esc_html__( 'White', 'mrseo' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'mrseo' ),
							'value'      => array(
								esc_html__( 'Default', 'mrseo' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'mrseo' ) ),
							'group'      => esc_html__( 'Product Info Style', 'mrseo' )
						)
					)
				)
	...
	public function getItemTitleHtml($params){
		...
		if(!empty($title)) {
			$html = '<'.esc_attr($title_tag).' itemprop="name" class="eltdf-pi-title entry-title" '.mrseo_elated_get_inline_style($product_info_style).'>';
    ...
    */
			...
			$html = '<div class="eltdf-pi-category">';
				foreach ($categories as $cat) {
					if(!empty($params['category_tag'])) {
						$html .= '<'.esc_attr($params['category_tag']).' '.mrseo_elated_get_inline_style($product_info_style).'>';
						...
						$html .= '<a itemprop="url" class="eltdf-pi-category-item" href="'.esc_url(get_term_link($cat->term_id)).'" '.mrseo_elated_get_inline_style($product_info_style).'>'.esc_html($cat->name).'</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml($params){
		...
		if(!empty($excerpt)) {
			$html = '<div class="eltdf-pi-excerpt"><p itemprop="description" class="eltdf-pi-excerpt-item" '.mrseo_elated_get_inline_style($product_info_style).'>'.esc_html($excerpt).'</p></div>';
		}
	...
	public function getItemPriceHtml($params){
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltdf-pi-price" '.mrseo_elated_get_inline_style($product_info_style).'>'.$price_html.'</div>';
		}
	...
	public function getItemRatingHtml($params){
		...
		if (get_option('woocommerce_enable_review_rating') !== 'no') {
			...
			$html = '<div class="eltdf-pi-rating" title="'.sprintf(esc_attr__("Rated %s out of 5", "mrseo"), $average ).'" '.mrseo_elated_get_inline_style($product_info_style).'><span style="width:'.(($average / 5)*100).'%"></span></div>';
		}
	...
	public function getItemAddToCartHtml($params){
		...
		$html .= apply_filters( 'mrseo_elated_product_info_add_to_cart_link',
	...
	public function getItemTagHtml($params){
		...
		if(!empty($tags)) {
			...
			foreach ($tags as $tag) {
				$html .= '<a itemprop="url" class="eltdf-pi-tag-item" href="'.esc_url(get_term_link($tag->term_id)).'" '.mrseo_elated_get_inline_style($product_info_style).'>'.esc_html($tag->name).'</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml($params){
		...
		if(!empty($author)) {
			$html = '<div class="eltdf-pi-author" '.mrseo_elated_get_inline_style($product_info_style).'>'.esc_html($author).'</div>';
		}
	...
	public function getItemDateHtml($params){
		...
		if(!empty($date)) {
			$html = '<div class="eltdf-pi-date" '.mrseo_elated_get_inline_style($product_info_style).'>'.esc_html($date).'</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'mrseo' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'mrseo' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $portfolio_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'mrseo' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'mrseo' ) . ': ' . $product_id;
